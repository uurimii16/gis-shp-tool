...
| |웹(이 폴더 `app.py`)|데스크톱(`desktop/`)|
|---|---|---|
|주소|https://gis-shp-tool.streamlit.app|내 PC에서 실행 / exe 배포|
|파일|업로드·다운로드|내 폴더에서 바로 읽고 바로 저장|
|대용량|업로드 용량·연결 끊김 제한|제한 없음|

기능(변환·병합·분할·코드결합)과 GDAL 처리 로직은 **양쪽이 동일**합니다.
데스크톱 사용법·exe 빌드는 [`desktop/README.md`](desktop/README.md) 참고.
...
- **`-append`는 재투영을 하지 않습니다.** 좌표계가 다른 레이어를 목표 EPSG 없이 붙이면
  GDAL이 경고도 없이 원본 좌표값을 그대로 밀어넣습니다. feature 개수는 정확한데 도형만
  엉뚱한 위치에 찍혀서 "안 합쳐진 것처럼" 보입니다. → 이제 목표 EPSG가 비어 있고 입력
  좌표계가 서로 다르면 **첫 레이어 좌표계로 자동 통일**하고 로그에 남깁니다. 병합 실행
  전에도 레이어별 좌표계·도형 타입 표를 화면에 보여줍니다.
- **SHP은 한 파일에 점/선/면을 섞어 담지 못합니다.** 섞인 채로 SHP을 만들면 첫 레이어와
  타입이 다른 피처가 통째로 버려집니다(순서에 따라 결과가 뒤바뀜: 점이 먼저면 면이 전멸).
  → 이제 섞인 것을 미리 감지해 GPKG 레이어를 `-nlt GEOMETRY`로 만들고 GPKG로 저장합니다.
- 같은 컬럼명인데 타입이 다르면(`CODE` 정수 ↔ 문자) GDAL이 Warning만 내고 값을 0으로
  넣습니다. → 로그의 ERROR/Warning을 결과 화면에 요약 표시합니다.
- **파일명이 같은 SHP을 여러 개 올리면** 예전에는 나중 파일이 앞 파일을 조용히 덮어썼습니다
  (시군구별 `연속지적도.shp` 등). → 이제 `중복2/` 하위 폴더로 분리 저장하고 경고합니다.
  가장 확실한 방법은 폴더째 ZIP으로 묶어 올리는 것입니다.

### 면적(area_m2) 산정 원리와 정확도

`ST_Area(geometry)`는 **평면(데카르트) 면적**입니다. 폴리곤 꼭짓점 좌표에 신발끈(shoelace)
공식을 적용해 **좌표 단위의 제곱**을 냅니다. 지구 곡률은 보지 않습니다.

- 좌표 단위가 m면 결과는 ㎡ → 계산 자체는 오차 0
- 경위도(EPSG:4326)면 "제곱도"라 무의미 → 앱이 경고 표시

정확도를 좌우하는 건 알고리즘이 아니라 **어떤 좌표계에서 재느냐**입니다.
같은 땅(1,000m × 1,000m = 1,000,000㎡)을 좌표계만 바꿔 측정한 실측값(GDAL 3.7):

|좌표계|계산된 면적|차이|
|---|---|---|
|EPSG:5186 (중부원점, k₀=1)|1,000,000 ㎡|0 (기준)|
|EPSG:5179 (UTM-K, k₀=0.9996)|999,249 ㎡|**−751㎡ (−0.075%)**|
|EPSG:5174 (구 중부원점)|1,000,013 ㎡|+13㎡|
|EPSG:4326 (경위도)|0.0000996 (제곱도)|무의미|

실무 주의사항:

- **5179(UTM-K)로 재면 항상 작게 나옵니다.** 축척계수 0.9996 → 면적 0.9996² = 약 0.08% 축소
  (1만㎡당 −7.5㎡). 국가공간정보포털·브이월드의 연속지적도가 대부분 5179이므로,
  면적을 낼 때는 목표 EPSG를 **5186**(또는 해당 원점 5185/5187)으로 통일하세요.
- 5186도 중앙자오선에서 멀어지면 커집니다. 벨트 끝(±88km)에서 이론상 +0.019%
  (1만㎡당 +1.9㎡). 실무상 무시 가능.
- **가장 큰 오차는 좌표계가 아니라 원본 도면입니다.** 연속지적도는 도면을 이어붙인
  *도면상* 도형이라 토지대장의 법정 면적과 필지당 수 %까지 차이 납니다. 법적 효력이 있는
  건 대장 면적이고, GIS 면적은 참고·집계용입니다. 소수점 자리수를 늘려도 이 오차는
  줄지 않으므로 보고서용이면 0~1자리를 권합니다.
- `area_sum`은 `ST_Area(ST_Union(geometry))`라 폴리곤이 겹쳐도 겹친 부분을 한 번만 셉니다
  (단순 `SUM`은 겹침을 중복 계산해 과대평가). 단 union 전 `-makevalid` 도형 복구가
  들어가므로, 원본이 깨진 도형이면 면적이 미세하게 달라질 수 있습니다.
...
- 면적 소수점 자리수 선택(변환·병합 탭, 0~6자리). **SHP 저장 시 실제 자리수 고정(2026-07-28 수정)** — `ROUND()`만으로는 DBF 필드가 기본값(폭 24/소수점 15)으로 만들어져 `11027.299999999999272`처럼 저장됨. 최종 저장 때 `-dialect OGRSQL`로 `CAST(x AS numeric(w,d))`를 걸어 필드 정의 자체를 바꿈(`decimal_cast_args`)
- 병합 안전장치(2026-07-28): 좌표계 자동 통일, 도형 타입 혼합 감지 → GPKG 저장, 동일 파일명 업로드 분리, ERROR/Warning 화면 요약
